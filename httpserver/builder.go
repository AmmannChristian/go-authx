...
func (b *ValidatorBuilder) Build() (TokenValidator, error) {
	...
	if b.useOpaqueToken {
		...
		if err != nil {
			return nil, fmt.Errorf("httpserver: failed to build opaque token validator: %w", err)
		}

		if b.logger != nil {
			b.logger.Printf("httpserver: using opaque token introspection endpoint: %s", b.introspectionURL)
		}
