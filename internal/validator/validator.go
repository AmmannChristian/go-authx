...
func NewJWTTokenValidator(jwksURL, issuer, audience string, httpClient *http.Client, cacheTTL time.Duration, logger Logger, logPrefix string) (*JWTTokenValidator, error) {
	if jwksURL == "" {
		return nil, errors.New("validator: JWKS URL is required")
	}
	if issuer == "" {
	...
	options := keyfunc.Options{
		RefreshErrorHandler: func(err error) {
			if logger != nil {
				logger.Printf("%s: JWKS refresh error: %v", logPrefix, err)
			}
		},
	...
	if err != nil {
		return nil, fmt.Errorf("validator: failed to initialize JWKS: %w", err)
	}
...
func (v *JWTTokenValidator) ValidateToken(ctx context.Context, tokenString string) (*TokenClaims, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("validator: token validation failed: %w", err)
	}

	if !token.Valid {
		return nil, errors.New("validator: token is invalid")
	}

	// Extract standard claims
	claims, ok := token.Claims.(jwt.MapClaims)
	if !ok {
		return nil, errors.New("validator: failed to extract token claims")
	}

	// Verify issuer
	iss, err := claims.GetIssuer()
	if err != nil || iss != v.issuer {
		return nil, fmt.Errorf("validator: invalid issuer: expected %s, got %s", v.issuer, iss)
	}

	// Verify audience
	aud, err := claims.GetAudience()
	if err != nil {
		return nil, fmt.Errorf("validator: invalid audience claim: %w", err)
	}
	if !contains(aud, v.audience) {
		return nil, fmt.Errorf("validator: invalid audience: expected %s in %v", v.audience, aud)
	}

	// Extract subject
	sub, err := claims.GetSubject()
	if err != nil {
		return nil, fmt.Errorf("validator: invalid subject claim: %w", err)
	}
	if sub == "" {
		return nil, errors.New("validator: invalid subject claim: empty")
	}

	// Extract expiry
	exp, err := claims.GetExpirationTime()
	if err != nil {
		return nil, fmt.Errorf("validator: invalid expiry claim: %w", err)
	}
	if exp == nil {
		return nil, errors.New("validator: invalid expiry claim: missing")
	}

	// Extract issued at
	iat, err := claims.GetIssuedAt()
	if err != nil {
		return nil, fmt.Errorf("validator: invalid issued at claim: %w", err)
	}
	if iat == nil {
		return nil, errors.New("validator: invalid issued at claim: missing")
	}
	...
	if v.logger != nil {
		v.logger.Printf("validator: validated token for subject %s with scopes %v", sub, scopes)
	}
